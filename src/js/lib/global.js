...
GLOBAL.App = function () {
    ...
    clientInteraction: function (args) {//点知天下客户端调用方法
        var u = navigator.userAgent,
            ua = navigator.userAgent.toLowerCase();
        if (u.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/)) {
            // 苹果
            window.webkit.messageHandlers.JSToNative_iOS.postMessage(args);
        } else {
            // 安卓
            window.JSToNative.postMessage(JSON.stringify(args));
        }
    },
    toast: function (text) {
        var obj = {
